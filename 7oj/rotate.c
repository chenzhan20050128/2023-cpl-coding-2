...
//int finalsum[MAX_N + MAX_M][MAX_N + MAX_M];
int de[MAX_N][MAX_M];
...
//         for (int j = 0; j < m; j++)
//         {
//             // de[i][j] = finalsum[(i - j + n - 1) / 2][(i + j - n + 1) / 2];
//             printf("%d ", de[i][j]);
//         }
//         printf("\n");
//     }
...
signed main()
{
    ...
    for (int i = 2 * d; i < n + m; i++)
    {
        for (int j = 2 * d; j < n + m; j++)
        {
            ...
            //finalsum[i][j] = sum;
            de[(i - j + n - 1) / 2][(i + j - n + 2 - 2 * d) / 2] = sum;
    ...
    //             int now_i = i - d;
    //             int now_j = j - d;
    //             answer[answer_cnt][0] = (now_i - now_j + n - 1) / 2 + 1;
    //             answer[answer_cnt][1] = (now_i + now_j - n + 2) / 2;
    //         }
    //     }
    // }
    myprint();
